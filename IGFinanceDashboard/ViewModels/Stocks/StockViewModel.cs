...
//using IGShowcase.FinanceDashboard.Services;
...
namespace IGShowcase.FinanceDashboard.ViewModels
{
    public class StockViewModel : ObservableModel 
	{
        ...
        public bool IsStockMatching(string search, StockSearchItemViewModel stockItem)
        {
            ...
            if (stockItem != null)
            {
                if (stockItem.Symbol.ToLower().StartsWith(search))
                    return true;
                var words = stockItem.NameShort.ToLower().Split(' ').ToList();
        ...
        public bool IsValidStockSymbol(string symbol)
        {
            ...
            //// check if the symbol is actually an existing stock
            //var isExistingStock = CheckForExistingStockBySymbolName(symbol);
            //if (isExistingStock.HasValue && isExistingStock.Value == false)
            //{
            //    return false;
            //}
		...
		{
			_stockService.RefreshDetails(_stocks.Select(x => x.Symbol).ToList(), OnRefreshDetails);
		}
		...
		{
            _stockService.RefreshData(stock.Symbol, DateTime.Now.AddYears(-100), OnRefreshData);
		}
        ...
        public void SimulateStockChanges()
        {
            foreach (var stock in _stocks)
            {
                if (stock.Details.IsStockTrading)
                {
                    ...
                    //var currentDetails = stock.Details;
                    ...
                    //if (currentDetails.DailyLow > currentDetails.LastTradeAmount)
                    //    currentDetails.DailyLow = currentDetails.LastTradeAmount;
                    //if (currentDetails.DailyHigh < currentDetails.LastTradeAmount)
                    //    currentDetails.DailyHigh = currentDetails.LastTradeAmount;

                    //currentDetails.UpdateTradeChange();
                    ////currentDetails.Volume = Math.Round(currentDetails.Volume * (Random.NextDouble() / 5 + 0.90), 2);
                    ...
                    //stock.UpdateLastStockData(currentDetails);
                }
            }
        }
		...
		private void OnRefreshDetails(IDictionary<string, StockTickerDetails> data)
        {
            ...
            foreach (var stock in _stocks)
			{
                ...
                {   
                    ...
                    if (!stock.Details.IsStockTrading)
                    {
                        stock.Details.Name = stock.Symbol + " " + Strings.Stock_DataIsMissing;
                        continue;
                    }
